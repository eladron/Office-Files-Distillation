...
namespace docx
{
  ...
  void Run::AppendDrawing(pugi::xml_node& drawing_node){
    std::cout << "drawing_node" << std::endl;
    w_r_.append_copy(drawing_node);
  }

  void Run::AppendObject(pugi::xml_node& object_node){
    std::cout << "object_node" << std::endl;
    w_r_.append_copy(object_node);
  }
  ...
  void Run::SetFontStyle(const FontStyle f, std::string val)
  {
    ...
    if (f & Underline) {
      if (u.empty())
        w_rPr_.append_child("w:u").append_attribute("w:val") = val.c_str();
    } else {
